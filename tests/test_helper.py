...
from enigma.helper import is_single_ascii_uppercase_letter, get_letter_index
import pytest


class TestIsSingleASCIIUppercaseLetter:
    @pytest.mark.parametrize("letter", ascii_uppercase)
    def test_with_ascii_uppercase_letter_should_return_true(self, letter) -> None:
...
class TestGetLetterIndex:
    @pytest.mark.parametrize(("expected_index", "letter"), enumerate(ascii_uppercase))
